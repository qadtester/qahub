...
def parse_ai_json(raw_text: str) -> Optional[Union[Dict[str, Any], list]]:
    """Trata respostas de IA para converter em JSON válido com varredura avançada e tolerância a texto livre."""
    if not raw_text:
        return None

    texto = raw_text.strip()
    
    # 1. Remove blocos de código Markdown se existirem de forma genérica
    if "```" in texto:
        parts = texto.split("```")
        for part in parts:
            p_limpo = part.strip()
    ...
    # 2. Varre os delimitadores estruturais mais externos (Objeto ou Array)
    ...
    # 3. Tenta carregar o JSON com correções de escape comuns geradas por LLMs
    try:
        return json.loads(texto)
    except Exception:
        try:
            # Fallback agressivo: limpa quebras de linha cruas dentro de strings que quebram o json.loads
            cleaned_text = texto.replace("\r", " ").replace("\n", "\\n")
            return json.loads(cleaned_text)
        except Exception as e2:
            # Se falhar totalmente, exibe a saída exata da IA para auditoria visual na interface
            st.error(f"⚠️ A IA não retornou um formato válido para a estrutura JSON: {e2}")
            with st.expander("🔍 Inspecionar resposta bruta da IA"):
                st.code(raw_text, language="text")
            return None

# ------------------------------------------------------------------------------
# CONSOLIDADOR DE CONTEXTO DO PROJETO
# ------------------------------------------------------------------------------

def get_full_project_context(project_id: str) -> str:
    """Busca o projeto e todos os documentos/textos salvos na tabela project_documents, unificando em um único contexto."""
    if not project_id:
        return ""
...
# ------------------------------------------------------------------------------
# GERADOR DE CONTEÚDO ISTQB
# ------------------------------------------------------------------------------

def generate_istqb_content(entity_type: str, user_context_or_project_id: str) -> Optional[Union[Dict[str, Any], list]]:
    """
    Função genérica para criar documentos de QA estruturados baseados em normas ISTQB.
    """
    ...
    2. RETORNE APENAS O JSON PURO. NENHUMA palavra, saudação, explicação ou bloco de código em markdown (como ```json) deve ser incluído antes ou depois do JSON. Comece diretamente com {{ ou [.
    """
    ...
    try:
        ...
        if not raw_response:
            st.error(f"⚠️ A API de IA retornou uma resposta vazia para o tipo: {entity_type}")
            return None
            
        return parse_ai_json(raw_response)
        
    except Exception as e:
        st.error(f"⚠️ Erro crítico ao chamar a API de IA: {e}")
        return None